...
| `fastMode` | boolean | No | Enables Codex Fast mode. Supported on `gpt-5.4`, `gpt-5.5`, and manual model IDs; burns credits faster |
...
Paperclip applies that when the selected model is `gpt-5.4`, `gpt-5.5`, or a manual model ID. On other models, the toggle is preserved in config but ignored at execution time to avoid unsupported runs.
